...
namespace luna
{
	namespace gfx
	{
		namespace d3d12
		{
			...
			{
				m_desc = desc;
				luassert_msg_usr(m_desc.access_type != EAccessType::unknown, "Cannot use EAccessType::unknown to create resource heap.");
				D3D12_HEAP_DESC d;
				...
				if (m_device->m_feature_options.ResourceHeapTier == D3D12_RESOURCE_HEAP_TIER_1)
				{
					...
					{
						set_err(e_bad_system_call, "On D3D12 Resource Heap Tier 1 hardware, only one type of EResourceHeapUsageFlag::buffer, EResourceHeapUsageFlag::texture_rt_ds or EResourceHeapUsageFlag::texture_non_rt_ds can be specified for one heap.");
			...
			RP<IResource> ResourceHeap::new_resource(uint64 offset, const ResourceDesc& desc, const ClearValue* optimized_clear_value)
			{
				lutsassert();
				// Check requirements.
				if (desc.access_type != EAccessType::unknown)
				{
					...
					{
						set_err(e_bad_arguments, "IResourceHeap::new_resource - Resource access type dismatch: Heap is %s while resource is %s", print_access_type(m_desc.access_type), print_access_type(desc.access_type));
						return e_user_failure;
					}
				}
				...
				{
					set_err(e_bad_arguments, "IResourceHeap::new_resource - Buffer resources are not allowed to be created in this heap.");
					return e_user_failure;
				}
				if (desc.type == EResourceType::texture_1d || desc.type == EResourceType::texture_2d || desc.type == EResourceType::texture_3d)
				{
					...
					{
						set_err(e_bad_arguments, "IResourceHeap::new_resource - Multi-sample textures are not allowed to be created in this heap.");
						return e_user_failure;
					}
					...
					{
						set_err(e_bad_arguments, "IResourceHeap::new_resource - Non-rt and non-ds textures are not allowed to be created in this heap.");
						return e_user_failure;
					}
					...
					{
						set_err(e_bad_arguments, "IResourceHeap::new_resource - RT and DS textures are not allowed to be created in this heap.");
						return e_user_failure;
					}
				}
				if (desc.type == EResourceType::unknown)
				{
					set_err(e_bad_arguments, "IResourceHeap::new_resource - The resource type is not allowed to be EResourceType::unknown when creating resources.");
					return e_user_failure;
				}
				//! Create desc object.
				...
				Resource* robj = new_obj<Resource>();
				if (!robj)
				{
