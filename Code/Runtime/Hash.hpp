...
namespace Luna
{
	...
	inline constexpr _HashTy memhash(const void* data, usize size, _HashTy h = 0)
	{
		size_t i{ 0 };
	...
	inline constexpr u32 memhash32(const void* data, usize size, u32 h = 0)
	{
		return memhash<u32>(data, size, h);
	}

	//! A specialization of `memhash` that uses uint64 value.
	inline constexpr u64 memhash64(const void* data, usize size, u64 h = 0)
	{
