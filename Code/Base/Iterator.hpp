...
namespace luna
{
	//! These tags are compatible to STLs.

	//! Supports equally-comparison(i == j, i != j), dereference(*i, i->m), increment(++i, i++, *i++).
	//struct input_iterator_tag {};
	//! With multi-pass guarantee, which supports assignment(_Iter j = i and j == i), and the increment
	//! operation for i will not change or discard the former value (so it can still be accessed by
	//! other copies of i).
	//! Can be used in multi-pass algorithms.
	//struct forward_iterator_tag : public input_iterator_tag {};
	//! Supports decrement(--i, i--, *i--)
	//struct bidirectional_iterator_tag : public forward_iterator_tag {};
	//! Supports add(i+n, n+i, i+=n), subtract(i-n, i-=n), difference(i-j), bracket-dereference(i[n])
	//! comparison(i>j, i<j, i>=j, i<=j).
	//struct random_access_iterator_tag : public bidirectional_iterator_tag {};
	//! Represents a contiguous range on memory ( *(i + n) equals to *(addressof(*i) + n) ).
	...
	public:
		...
		constexpr ReverseIterator operator+(ptrdiff_t n) const
		{
			return ReverseIterator(m_base - n);
		}

		constexpr ReverseIterator& operator+=(ptrdiff_t n)
		{
		...
		constexpr reference operator[](ptrdiff_t n) const
		{
			return m_base[-n - 1];
		}
	};
	...
	template <typename _Iter1, typename _Iter2>
	constexpr inline typename ReverseIterator<_Iter1>::difference_type
		operator-(const ReverseIterator<_Iter1>& a, const ReverseIterator<_Iter2>& b)
	{
	...
	constexpr inline ReverseIterator<_Iter>
		operator+(ptrdiff_t n, const ReverseIterator<_Iter>& a)
	{
