...
namespace luna
{
	...
	{
		new (static_cast<void*>(addressof(*dest)))
			typename iterator_traits<_Iter1>::value_type(forward<_Args>(args)...);
	}
	...
	//! If `value_type` of `_Iter2` is trivially move constructible, calls `copy_construct_range` to do the move
	//! construct.
	...
	//! If `value_type` is trivially destructible, do nothing.
	...
	{
		template <typename _Ty1, typename _Ty2>
		inline _Ty2* move_relocate_range_backward_virtual(_Ty1* first, _Ty1* last, _Ty2* d_last, true_type)	// is_pointer
		{
			ptrdiff_t sz = last - first;
			memmove(static_cast<void*>(d_last - sz), static_cast<const void*>(first), (size_t)(sz) * sizeof(_Ty2));
			return d_last - sz;
		...
		template <typename _Iter1, typename _Iter2>
		inline _Iter2 move_relocate_range_backward_virtual(_Iter1 first, _Iter1 last, _Iter2 d_last, false_type) // not is_pointer
		{
			while (first != last)
			{
				--last;
		...
		{
			using is_iterator_pointer = typename conjunction<is_pointer<_Iter1>, is_pointer<_Iter2>>::type;
			return move_relocate_range_backward_virtual(first, last, d_last, is_iterator_pointer());
		}
