...
namespace luna
{
	namespace font
	{
		enum class EFontFileFormat : uint32
		{
			ttf = 1,	// True Type Font, including .ttf and .ttc
		};

		LUNA_FONT_API void init();
		...
		//! font data.
		LUNA_FONT_API RP<IFontFile> load_font_file(IBlob* data, EFontFileFormat format);
		...
		LUNA_FONT_API RP<IFontAtlas> new_font_atlas(IFontFile* fontfile, uint32 font_index, float32 font_height);
