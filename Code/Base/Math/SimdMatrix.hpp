...
namespace luna
{
	namespace simd
	{
		...
		)
		{
			...
			vec_float_t vX = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[0]) + 0);
			vec_float_t vY = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[0]) + 1);
			vec_float_t vZ = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[0]) + 2);
			vec_float_t vW = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[0]) + 3);
#else
	// Use vW to hold the original row
			...
			// Repeat for the other 3 rows
#if defined(LUNA_PLATFORM_AVX)
			vX = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[1]) + 0);
			vY = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[1]) + 1);
			vZ = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[1]) + 2);
			vW = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[1]) + 3);
#else
			vW = M1.r[1];
			...
			mResult.r[1] = vX;
#if defined(LUNA_PLATFORM_AVX)
			vX = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[2]) + 0);
			vY = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[2]) + 1);
			vZ = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[2]) + 2);
			vW = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[2]) + 3);
#else
			vW = M1.r[2];
			vX = LUNA_SIMD_PERMUTE_PS(vW, _MM_SHUFFLE(0, 0, 0, 0));
			vY = LUNA_SIMD_PERMUTE_PS(vW, _MM_SHUFFLE(1, 1, 1, 1));
			...
			mResult.r[2] = vX;
#if defined(LUNA_PLATFORM_AVX)
			vX = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[3]) + 0);
			vY = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[3]) + 1);
			vZ = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[3]) + 2);
			vW = _mm_broadcast_ss(reinterpret_cast<const float32*>(&M1.r[3]) + 3);
#else
			vW = M1.r[3];
			vX = LUNA_SIMD_PERMUTE_PS(vW, _MM_SHUFFLE(0, 0, 0, 0));
			vY = LUNA_SIMD_PERMUTE_PS(vW, _MM_SHUFFLE(1, 1, 1, 1));
...
#define LUNA_SIMD_3_DECOMP_EPSILON 0.0001f
		...
		)
		{
			...
			auto pfScales = reinterpret_cast<float32 *>(outScale);
			...
			float32 fDet = VectorGetX(MatrixDeterminant(matTemp));
...
#undef LUNA_SIMD_3RANKDECOMPOSE
		...
		{
			vec_float_t Angles = VectorSet(Pitch, Yaw, Roll, 0.0f);
		...
		)
		{
			...
			vec_float_t A = VectorSet(fSinAngle, fCosAngle, 1.0f - fCosAngle, 0.0f);

			vec_float_t C2 = VectorSplatZ(A);
			vec_float_t C1 = VectorSplatY(A);
			vec_float_t C0 = VectorSplatX(A);
		...
		{
			luassert(!Vector3Equal(Axis, VectorZero()));
		...
		{
			luassert(!Vector3Equal(ReflectionPlane, VectorZero()));
			...
			vec_float_t A = VectorSplatX(P);
			vec_float_t B = VectorSplatY(P);
			vec_float_t C = VectorSplatZ(P);
			vec_float_t D = VectorSplatW(P);
		...
		)
		{
			...
			luassert(!Vector3Equal(ShadowPlane, VectorZero()));
		...
		{
			luassert(!Vector3Equal(EyeDirection, VectorZero()));
			luassert(!Vector3IsInfinite(EyeDirection));
			luassert(!Vector3Equal(UpDirection, VectorZero()));
...
#if defined(LUNA_PLATFORM_ARM_NEON)
			float32 TwoNearZ = NearZ + NearZ;
			float32 fRange = FarZ / (NearZ - FarZ);
			const vec_float_t Zero = vdupq_n_f32(0);
...
#if defined(LUNA_PLATFORM_ARM_NEON)
			float32 TwoNearZ = NearZ + NearZ;
			float32 ReciprocalWidth = 1.0f / (ViewRight - ViewLeft);
			float32 ReciprocalHeight = 1.0f / (ViewTop - ViewBottom);
			float32 fRange = FarZ / (FarZ - NearZ);
			const vec_float_t Zero = vdupq_n_f32(0);
...
#if defined(LUNA_PLATFORM_ARM_NEON)
			float32 fRange = 1.0f / (NearZ - FarZ);
...
#elif defined(LUNA_PLATFORM_SSE2)
			mat_float_t M;
			float32 fReciprocalWidth = 1.0f / (ViewRight - ViewLeft);
			float32 fReciprocalHeight = 1.0f / (ViewTop - ViewBottom);
			float32 fRange = 1.0f / (FarZ - NearZ);
			// Note: This is recorded on the stack
			vec_float_t rMem = {
				fReciprocalWidth,
...
#pragma prefast(pop)
		...
		inline mat_float_t::mat_float_t
		(
			const float32* pArray
		)
		{
			luassert(pArray != nullptr);
			r[0] = load_float4(reinterpret_cast<const float32*>(pArray));
			r[1] = load_float4(reinterpret_cast<const float32*>(pArray + 4));
			r[2] = load_float4(reinterpret_cast<const float32*>(pArray + 8));
			r[3] = load_float4(reinterpret_cast<const float32*>(pArray + 12));
		}
		...
		inline mat_float_t& mat_float_t::operator*= (float32 S)
		{
		...
		inline mat_float_t mat_float_t::operator* (float32 S) const
		{
			mat_float_t R;
