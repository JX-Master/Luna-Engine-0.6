...
namespace luna
{
	struct alignas(Float4) Quaternion
	{
		...
		constexpr Quaternion(float32 _x, float32 _y, float32 _z, float32 _w) :
			x(_x), y(_y), z(_z), w(_w) {}
		Quaternion(const Float3& v, float32 scalar) :
			x(v.x), y(v.y), z(v.z), w(scalar) {}
		explicit Quaternion(const Float4& v) :
			x(v.x), y(v.y), z(v.z), w(v.w) {}
		explicit Quaternion(const float32* arr)
		{
		...
		static Quaternion from_axis_angle(const Float3& axis, float32 angle);
		static Quaternion from_euler_angles(const Float3& euler_angles);
		static Quaternion from_roll_pitch_yaw(float32 pitch, float32 yaw, float32 roll);
	...
	Quaternion operator- (const Quaternion& q1, const Quaternion& q2);
	Quaternion operator* (const Quaternion& q1, const Quaternion& q2);
	Quaternion operator* (const Quaternion& q, float32 s);
	Quaternion operator/ (const Quaternion& q1, const Quaternion& q2);
	Quaternion operator* (float32 s, const Quaternion& q);

	float32 length(const Quaternion& q);
	float32 length_squared(const Quaternion& q);
	Quaternion normalize(const Quaternion& q);
	Quaternion conjugate(const Quaternion& q);
	Quaternion inverse(const Quaternion& q);
	float32 dot(const Quaternion& q1, const Quaternion& q2);
	Quaternion lerp(const Quaternion& q1, const Quaternion& q2, float32 t);
	...
	inline Quaternion operator* (const Quaternion& Q, float32 S)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
	...
	inline Quaternion operator* (float32 S, const Quaternion& Q)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
	...
	inline float32 length_squared(Quaternion& q)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
		vec_float_t q_vec = load_float4a(q.m);
	...
	inline Quaternion Quaternion::from_axis_angle(const Float3 &axis, float32 angle)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
		Quaternion result;
		...
		float32 sinV = sinf(0.5f * angle);
		float32 cosV = cosf(0.5f * angle);
		Quaternion result;
	...
	inline Quaternion Quaternion::from_roll_pitch_yaw(float32 pitch, float32 yaw, float32 roll)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
		Quaternion result;
	...
	inline Quaternion lerp(const Quaternion& q1, const Quaternion& q2, float32 t)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
		...
		vec_float_t R;
		if (Vector4GreaterOrEqual(dot, VectorZero()))
		{
			R = VectorLerp(Q0, Q1, t);
		}
		else
		{
			vec_float_t tv = VectorReplicate(t);
			vec_float_t t1v = VectorReplicate(1.f - t);
	...
	inline Quaternion slerp(const Quaternion& q1, const Quaternion& q2, float32 t)
	{
#ifdef LUNA_SIMD_ENABLED 
		using namespace simd; 
		vec_float_t Q0 = load_float4a(q1.m);
		...
		float32 CosOmega = dot(q1, q2);
		CosOmega = CosOmega < 0 ? -CosOmega : CosOmega;
		float32 SinOmega = sqrtf(1.0f - CosOmega * CosOmega);
		float32 Omega = atan2f(SinOmega, CosOmega);
