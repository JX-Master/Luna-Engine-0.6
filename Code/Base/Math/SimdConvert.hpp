...
namespace luna
{
	namespace simd
	{
		...
		)
		{
			...
			float32 fScale = 1.0f / (float)(1U << DivExponent);
			...
			uint32 uScale = 0x3F800000U - (DivExponent << 23);
		...
		)
		{
			...
			return vOverflow;
#else // LUNA_PLATFORM_SSE2
			vec_float_t vResult = _mm_set_ps1(static_cast<float32>(1U << MulExponent));
		...
		****************************************************************************/
		...
		inline vec_float_t LUNA_SIMD_CALLCONV load_uint(const uint32* pSource)
		{
			luassert(pSource);
#if defined(LUNA_PLATFORM_ARM_NEON)
		...
		inline vec_float_t LUNA_SIMD_CALLCONV load_int(const int32* pSource)
		{
			luassert(pSource);
#if defined(LUNA_PLATFORM_ARM_NEON)
			uint32x4_t zero = vdupq_n_u32(0);
		...
		inline vec_float_t LUNA_SIMD_CALLCONV load_float(const float32* pSource)
		{
			luassert(pSource);
#if defined(LUNA_PLATFORM_ARM_NEON)
		...
		)
		{
			...
			luassert((reinterpret_cast<uintptr_t>(pSource) & 0xF) == 0);
#if defined(LUNA_PLATFORM_ARM_NEON)
			// Reads an extra float32 which is zero's
		...
		)
		{
			...
			float32x2_t v2 = vcreate_f32(static_cast<uint64_t>(*reinterpret_cast<const uint32*>(pSource + 8)));
