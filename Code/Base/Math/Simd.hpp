...
namespace luna
{
	...
	namespace simd
	{
		constexpr uint32 g_select_0 = 0x00000000;
		constexpr uint32 g_select_1 = 0xFFFFFFFF;

		constexpr uint32 g_permute_0x = 0;
		constexpr uint32 g_permute_0y = 1;
		constexpr uint32 g_permute_0z = 2;
		constexpr uint32 g_permute_0w = 3;
		constexpr uint32 g_permute_1x = 4;
		constexpr uint32 g_permute_1y = 5;
		constexpr uint32 g_permute_1z = 6;
		constexpr uint32 g_permute_1w = 7;

		constexpr uint32 g_swizzle_x = 0;
		constexpr uint32 g_swizzle_y = 1;
		constexpr uint32 g_swizzle_z = 2;
		constexpr uint32 g_swizzle_w = 3;

		constexpr uint32 g_crmask_cr6 = 0x000000F0;
		constexpr uint32 g_crmask_cr6true = 0x00000080;
		constexpr uint32 g_crmask_cr6false = 0x00000020;
		constexpr uint32 g_crmask_cr6bounds = g_crmask_cr6false;

		constexpr size_t g_cache_line_size = 64;

		// Condition register evaluation proceeding a recording (R) comparison

		inline bool comparison_all_true(uint32 CR) { return (((CR)& g_crmask_cr6true) == g_crmask_cr6true); }
		inline bool comparison_any_true(uint32 CR) { return (((CR)& g_crmask_cr6false) != g_crmask_cr6false); }
		inline bool comparison_all_false(uint32 CR) { return (((CR)& g_crmask_cr6false) == g_crmask_cr6false); }
		inline bool comparison_any_false(uint32 CR) { return (((CR)& g_crmask_cr6true) != g_crmask_cr6true); }
		inline bool comparison_mixed(uint32 CR) { return (((CR)& g_crmask_cr6) == 0); }
		inline bool comparison_all_in_bounds(uint32 CR) { return (((CR)& g_crmask_cr6bounds) == g_crmask_cr6bounds); }
		inline bool comparison_any_out_of_bounds(uint32 CR) { return (((CR)& g_crmask_cr6bounds) != g_crmask_cr6bounds); }
...
#error "Unsupported SIMD Target."
...
#else
		using vec_float_p1_t = const vec_float&;
		//using vec_int32_p1 = const vec_int32_t&;
		//using vec_uint32_p1 = const vec_uint32_t&;
#endif
...
#else
		using vec_float_p3_t = const vec_float&;
		//using vec_int32_p3 = const vec_int32_t&;
		//using vec_uint32_p3 = const vec_uint32_t&;
#endif
		...
		struct alignas(16) vec_f32_t
		{
			...
			inline operator const float32*() const { return f; }
#if defined(LUNA_PLATFORM_SSE2)
		...
		struct alignas(16) mat_float_t
		{
			...
			mat_float_t(float32 m00, float32 m01, float32 m02, float32 m03,
				float32 m10, float32 m11, float32 m12, float32 m13,
				float32 m20, float32 m21, float32 m22, float32 m23,
				float32 m30, float32 m31, float32 m32, float32 m33);
			explicit mat_float_t(const float32* pArray);
			...
			mat_float_t&   LUNA_SIMD_CALLCONV     operator-= (mat_float_p1_t M);
			mat_float_t&   LUNA_SIMD_CALLCONV     operator*= (mat_float_p1_t M);
			mat_float_t&   operator*= (float32 S);
			mat_float_t&   operator/= (float32 S);

			mat_float_t    LUNA_SIMD_CALLCONV     operator+ (mat_float_p1_t M) const;
			mat_float_t    LUNA_SIMD_CALLCONV     operator- (mat_float_p1_t M) const;
			mat_float_t    LUNA_SIMD_CALLCONV     operator* (mat_float_p1_t M) const;
			mat_float_t    operator* (float32 S) const;
			mat_float_t    operator/ (float32 S) const;

			friend mat_float_t     LUNA_SIMD_CALLCONV     operator* (float32 S, mat_float_p1_t M);
		};
		...
		vec_float_t LUNA_SIMD_CALLCONV convert_vector_int_to_float(vec_float_p1_t vint, uint32 divExponent);
		vec_float_t LUNA_SIMD_CALLCONV convert_vector_float_to_int(vec_float_p1_t vfloat, uint32 mulExponent);
		vec_float_t LUNA_SIMD_CALLCONV convert_vector_uint_to_float(vec_float_p1_t vuint, uint32 divExponent);
		vec_float_t LUNA_SIMD_CALLCONV convert_vector_float_to_uint(vec_float_p1_t vfloat, uint32 mulExponent);
		vec_float_t LUNA_SIMD_CALLCONV vector_set_binary_constant(uint32 c0, uint32 c1, uint32 c2, uint32 c3);
		vec_float_t LUNA_SIMD_CALLCONV vector_splat_constant(int32 intConstant, uint32 divExponent);
		vec_float_t LUNA_SIMD_CALLCONV vector_splat_constant_int(int32 intConstant);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     load_uint(const uint32* pSource);
		vec_float_t	   LUNA_SIMD_CALLCONV	  load_int(const int32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float(const float32* pSource);

		vec_float_t    LUNA_SIMD_CALLCONV     load_uint2(const uint32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_uint2a(const uint32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int2(const int32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int2a(const int32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float2(const float32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float2a(const float32* pSource);

		vec_float_t    LUNA_SIMD_CALLCONV     load_uint3(const uint32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_uint3a(const uint32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int3(const int32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int3a(const int32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float3(const float32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float3a(const float32* pSource);

		vec_float_t    LUNA_SIMD_CALLCONV     load_uint4(const uint32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_uint4a(const uint32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int4(const int32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_int4a(const int32* PSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float4(const float32* pSource);
		vec_float_t    LUNA_SIMD_CALLCONV     load_float4a(const float32* pSource);

		mat_float_t    LUNA_SIMD_CALLCONV     load_float3x3(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float4x3(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float4x3a(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float3x4(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float3x4a(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float4x4(const float32* pSource);
		mat_float_t    LUNA_SIMD_CALLCONV     load_float4x4a(const float32* pSource);
		...
		void        LUNA_SIMD_CALLCONV     store_int(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float(float32* pDestination, vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     store_int2(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_int2a(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint2(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint2a(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float2(float32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float2a(float32* pDestination, vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     store_int3(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_int3a(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint3(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint3a(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float3(float32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float3a(float32* pDestination, vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     store_int4(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_int4a(int32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint4(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_uint4a(uint32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float4(float32* pDestination, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     store_float4a(float32* pDestination, vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     store_float3x3(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float4x3(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float4x3a(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float3x4(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float3x4a(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float4x4(float32* pDestination, mat_float_p1_t M);
		void        LUNA_SIMD_CALLCONV     store_float4x4a(float32* pDestination, mat_float_p1_t M);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorZero();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSet(float32 x, float32 y, float32 z, float32 w);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetInt(uint32 x, uint32 y, uint32 z, uint32 w);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorReplicate(float32 Value);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorReplicatePtr(const float32* pValue);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorReplicateInt(uint32 Value);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorReplicateIntPtr(const uint32* pValue);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorTrueInt();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorFalseInt();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatX(vec_float_p1_t V);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatY(vec_float_p1_t V);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatZ(vec_float_p1_t V);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatW(vec_float_p1_t V);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatOne();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatInfinity();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatQNaN();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatEpsilon();
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSplatSignMask();

		float32       LUNA_SIMD_CALLCONV     VectorGetByIndex(vec_float_p1_t V, size_t i);
		float32       LUNA_SIMD_CALLCONV     VectorGetX(vec_float_p1_t V);
		float32       LUNA_SIMD_CALLCONV     VectorGetY(vec_float_p1_t V);
		float32       LUNA_SIMD_CALLCONV     VectorGetZ(vec_float_p1_t V);
		float32       LUNA_SIMD_CALLCONV     VectorGetW(vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     VectorGetByIndexPtr(float32* f, vec_float_p1_t V, size_t i);
		void        LUNA_SIMD_CALLCONV     VectorGetXPtr(float32* x, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetYPtr(float32* y, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetZPtr(float32* z, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetWPtr(float32* w, vec_float_p1_t V);

		uint32      LUNA_SIMD_CALLCONV     VectorGetIntByIndex(vec_float_p1_t V, size_t i);
		uint32      LUNA_SIMD_CALLCONV     VectorGetIntX(vec_float_p1_t V);
		uint32      LUNA_SIMD_CALLCONV     VectorGetIntY(vec_float_p1_t V);
		uint32      LUNA_SIMD_CALLCONV     VectorGetIntZ(vec_float_p1_t V);
		uint32      LUNA_SIMD_CALLCONV     VectorGetIntW(vec_float_p1_t V);

		void        LUNA_SIMD_CALLCONV     VectorGetIntByIndexPtr(uint32* x, vec_float_p1_t V, size_t i);
		void        LUNA_SIMD_CALLCONV     VectorGetIntXPtr(uint32* x, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetIntYPtr(uint32* y, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetIntZPtr(uint32* z, vec_float_p1_t V);
		void        LUNA_SIMD_CALLCONV     VectorGetIntWPtr(uint32* w, vec_float_p1_t V);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetByIndex(vec_float_p1_t V, float32 f, size_t i);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetX(vec_float_p1_t V, float32 x);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetY(vec_float_p1_t V, float32 y);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetZ(vec_float_p1_t V, float32 z);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetW(vec_float_p1_t V, float32 w);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetByIndexPtr(vec_float_p1_t V, const float32* f, size_t i);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetXPtr(vec_float_p1_t V, const float32* x);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetYPtr(vec_float_p1_t V, const float32* y);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetZPtr(vec_float_p1_t V, const float32* z);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetWPtr(vec_float_p1_t V, const float32* w);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntByIndex(vec_float_p1_t V, uint32 x, size_t i);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntX(vec_float_p1_t V, uint32 x);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntY(vec_float_p1_t V, uint32 y);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntZ(vec_float_p1_t V, uint32 z);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntW(vec_float_p1_t V, uint32 w);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntByIndexPtr(vec_float_p1_t V, const uint32* x, size_t i);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntXPtr(vec_float_p1_t V, const uint32* x);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntYPtr(vec_float_p1_t V, const uint32* y);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntZPtr(vec_float_p1_t V, const uint32* z);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSetIntWPtr(vec_float_p1_t V, const uint32* w);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorSwizzle(vec_float_p1_t V, uint32 E0, uint32 E1, uint32 E2, uint32 E3);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorPermute(vec_float_p1_t V1, vec_float_p1_t V2, uint32 PermuteX, uint32 PermuteY, uint32 PermuteZ, uint32 PermuteW);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorSelectControl(uint32 VectorIndex0, uint32 VectorIndex1, uint32 VectorIndex2, uint32 VectorIndex3);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorShiftLeft(vec_float_p1_t V1, vec_float_p1_t V2, uint32 Elements);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorRotateLeft(vec_float_p1_t V, uint32 Elements);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorRotateRight(vec_float_p1_t V, uint32 Elements);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorInsert(vec_float_p1_t VD, vec_float_p1_t VS, uint32 VSLeftRotateElements,
			uint32 Select0, uint32 Select1, uint32 Select2, uint32 Select3);

		vec_float_t    LUNA_SIMD_CALLCONV     VectorEqual(vec_float_p1_t V1, vec_float_p1_t V2);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorEqualR(uint32* pCR, vec_float_p1_t V1, vec_float_p1_t V2);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorEqualInt(vec_float_p1_t V1, vec_float_p1_t V2);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorEqualIntR(uint32* pCR, vec_float_p1_t V, vec_float_p1_t V2);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorGreaterR(uint32* pCR, vec_float_p1_t V1, vec_float_p1_t V2);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorGreaterOrEqual(vec_float_p1_t V1, vec_float_p1_t V2);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorGreaterOrEqualR(uint32* pCR, vec_float_p1_t V1, vec_float_p1_t V2);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorInBoundsR(uint32* pCR, vec_float_p1_t V, vec_float_p1_t Bounds);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorScale(vec_float_p1_t V, float32 ScaleFactor);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorLerp(vec_float_p1_t V0, vec_float_p1_t V1, float32 t);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorLerpV(vec_float_p1_t V0, vec_float_p1_t V1, vec_float_p1_t T);
		vec_float_t    LUNA_SIMD_CALLCONV     VectorHermite(vec_float_p1_t Position0, vec_float_p1_t Tangent0, vec_float_p1_t Position1, vec_float_p2_t Tangent1, float32 t);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorCatmullRom(vec_float_p1_t Position0, vec_float_p1_t Position1, vec_float_p1_t Position2, vec_float_p2_t Position3, float32 t);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     VectorBaryCentric(vec_float_p1_t Position0, vec_float_p1_t Position1, vec_float_p1_t Position2, float32 f, float32 g);
		...
		uint32    LUNA_SIMD_CALLCONV     Vector2EqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector2EqualInt(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector2EqualIntR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		uint32    LUNA_SIMD_CALLCONV     Vector2GreaterR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector2GreaterOrEqual(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector2GreaterOrEqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector2ClampLength(vec_float_p1_t V, float32 LengthMin, float32 LengthMax);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector2Refract(vec_float_p1_t Incident, vec_float_p1_t Normal, float32 RefractionIndex);
		...
		float32*   LUNA_SIMD_CALLCONV     Vector2TransformStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector2TransformCoord(vec_float_p1_t V, mat_float_p1_t M);
		float32*   LUNA_SIMD_CALLCONV     Vector2TransformCoordStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector2TransformNormal(vec_float_p1_t V, mat_float_p1_t M);
		float32*   LUNA_SIMD_CALLCONV     Vector2TransformNormalStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		...
		uint32    LUNA_SIMD_CALLCONV     Vector3EqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector3EqualInt(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector3EqualIntR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		uint32    LUNA_SIMD_CALLCONV     Vector3GreaterR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector3GreaterOrEqual(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector3GreaterOrEqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3ClampLength(vec_float_p1_t V, float32 LengthMin, float32 LengthMax);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3Refract(vec_float_p1_t Incident, vec_float_p1_t Normal, float32 RefractionIndex);
		...
		float32*   LUNA_SIMD_CALLCONV     Vector3TransformStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3TransformCoord(vec_float_p1_t V, mat_float_p1_t M);
		float32*   LUNA_SIMD_CALLCONV     Vector3TransformCoordStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3TransformNormal(vec_float_p1_t V, mat_float_p1_t M);
		float32*   LUNA_SIMD_CALLCONV     Vector3TransformNormalStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3Project(vec_float_p1_t V, float32 ViewportX, float32 ViewportY, float32 ViewportWidth, float32 ViewportHeight, float32 ViewportMinZ, float32 ViewportMaxZ,
			mat_float_p1_t Projection, mat_float_p2_t View, mat_float_p2_t World);
		float32*   LUNA_SIMD_CALLCONV     Vector3ProjectStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount,
			float32 ViewportX, float32 ViewportY, float32 ViewportWidth, float32 ViewportHeight, float32 ViewportMinZ, float32 ViewportMaxZ,
			mat_float_p1_t Projection, mat_float_p2_t View, mat_float_p2_t World);
		vec_float_t    LUNA_SIMD_CALLCONV     Vector3Unproject(vec_float_p1_t V, float32 ViewportX, float32 ViewportY, float32 ViewportWidth, float32 ViewportHeight, float32 ViewportMinZ, float32 ViewportMaxZ,
			mat_float_p1_t Projection, mat_float_p2_t View, mat_float_p2_t World);
		float32*   LUNA_SIMD_CALLCONV     Vector3UnprojectStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount,
			float32 ViewportX, float32 ViewportY, float32 ViewportWidth, float32 ViewportHeight, float32 ViewportMinZ, float32 ViewportMaxZ,
		...
		uint32    LUNA_SIMD_CALLCONV     Vector4EqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector4EqualInt(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector4EqualIntR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		uint32    LUNA_SIMD_CALLCONV     Vector4GreaterR(vec_float_p1_t V1, vec_float_p1_t V2);
		bool        LUNA_SIMD_CALLCONV     Vector4GreaterOrEqual(vec_float_p1_t V1, vec_float_p1_t V2);
		uint32    LUNA_SIMD_CALLCONV     Vector4GreaterOrEqualR(vec_float_p1_t V1, vec_float_p1_t V2);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector4ClampLength(vec_float_p1_t V, float32 LengthMin, float32 LengthMax);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     Vector4Refract(vec_float_p1_t Incident, vec_float_p1_t Normal, float32 RefractionIndex);
		...
		float32*   LUNA_SIMD_CALLCONV     Vector4TransformStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t VectorCount, mat_float_p1_t M);
		...
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixSet(float32 m00, float32 m01, float32 m02, float32 m03,
			float32 m10, float32 m11, float32 m12, float32 m13,
			float32 m20, float32 m21, float32 m22, float32 m23,
			float32 m30, float32 m31, float32 m32, float32 m33);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixTranslation(float32 OffsetX, float32 OffsetY, float32 OffsetZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixTranslationFromVector(vec_float_p1_t Offset);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixScaling(float32 ScaleX, float32 ScaleY, float32 ScaleZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixScalingFromVector(vec_float_p1_t Scale);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationX(float32 Angle);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationY(float32 Angle);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationZ(float32 Angle);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationRollPitchYaw(float32 Pitch, float32 Yaw, float32 Roll);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationRollPitchYawFromVector(vec_float_p1_t Angles);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationNormal(vec_float_p1_t NormalAxis, float32 Angle);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationAxis(vec_float_p1_t Axis, float32 Angle);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixRotationQuaternion(vec_float_p1_t Quaternion);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixTransformation2D(vec_float_p1_t ScalingOrigin, float32 ScalingOrientation, vec_float_p1_t Scaling,
			vec_float_p1_t RotationOrigin, float32 Rotation, vec_float_p2_t Translation);
		...
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixAffineTransformation2D(vec_float_p1_t Scaling, vec_float_p1_t RotationOrigin, float32 Rotation, vec_float_p1_t Translation);
		...
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveLH(float32 ViewWidth, float32 ViewHeight, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveRH(float32 ViewWidth, float32 ViewHeight, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveFovLH(float32 FovAngleY, float32 AspectRatio, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveFovRH(float32 FovAngleY, float32 AspectRatio, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveOffCenterLH(float32 ViewLeft, float32 ViewRight, float32 ViewBottom, float32 ViewTop, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixPerspectiveOffCenterRH(float32 ViewLeft, float32 ViewRight, float32 ViewBottom, float32 ViewTop, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixOrthographicLH(float32 ViewWidth, float32 ViewHeight, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixOrthographicRH(float32 ViewWidth, float32 ViewHeight, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixOrthographicOffCenterLH(float32 ViewLeft, float32 ViewRight, float32 ViewBottom, float32 ViewTop, float32 NearZ, float32 FarZ);
		mat_float_t    LUNA_SIMD_CALLCONV     MatrixOrthographicOffCenterRH(float32 ViewLeft, float32 ViewRight, float32 ViewBottom, float32 ViewTop, float32 NearZ, float32 FarZ);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionSlerp(vec_float_p1_t Q0, vec_float_p1_t Q1, float32 t);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionSlerpV(vec_float_p1_t Q0, vec_float_p1_t Q1, vec_float_p1_t T);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionSquad(vec_float_p1_t Q0, vec_float_p1_t Q1, vec_float_p1_t Q2, vec_float_p2_t Q3, float32 t);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionBaryCentric(vec_float_p1_t Q0, vec_float_p1_t Q1, vec_float_p1_t Q2, float32 f, float32 g);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionRotationRollPitchYaw(float32 Pitch, float32 Yaw, float32 Roll);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionRotationRollPitchYawFromVector(vec_float_p1_t Angles);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionRotationNormal(vec_float_p1_t NormalAxis, float32 Angle);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionRotationAxis(vec_float_p1_t Axis, float32 Angle);
		vec_float_t    LUNA_SIMD_CALLCONV     QuaternionRotationMatrix(mat_float_p1_t M);

		void        LUNA_SIMD_CALLCONV     QuaternionToAxisAngle(vec_float_t* pAxis, float32* pAngle, vec_float_p1_t Q);
		...
		float32*   LUNA_SIMD_CALLCONV     PlaneTransformStream(float32* pOutputStream,
			size_t OutputStride,
			const float32* pInputStream,
			size_t InputStride, size_t PlaneCount, mat_float_p1_t M);
		...
		vec_float_t    LUNA_SIMD_CALLCONV     ColorAdjustSaturation(vec_float_p1_t C, float32 Saturation);
		vec_float_t    LUNA_SIMD_CALLCONV     ColorAdjustContrast(vec_float_p1_t C, float32 Contrast);
		...
		bool            ScalarNearEqual(float32 S1, float32 S2, float32 Epsilon);
		float32           ScalarModAngle(float32 Value);

		float32           ScalarSin(float32 Value);
		float32           ScalarSinEst(float32 Value);

		float32           ScalarCos(float32 Value);
		float32           ScalarCosEst(float32 Value);

		void            ScalarSinCos(float32* pSin, float32* pCos, float32 Value);
		void            ScalarSinCosEst(float32* pSin, float32* pCos, float32 Value);

		float32           ScalarASin(float32 Value);
		float32           ScalarASinEst(float32 Value);

		float32           ScalarACos(float32 Value);
		float32           ScalarACosEst(float32 Value);
...
// PermuteHelper internal template (SSE only)
		namespace Internal
		{
			...
			template<uint32 Shuffle, bool WhichX, bool WhichY, bool WhichZ, bool WhichW> struct PermuteHelper
			{
				static vec_float_t     LUNA_SIMD_CALLCONV     Permute(vec_float_p1_t v1, vec_float_p1_t v2)
				{
					static alignas(16) const uint32 selectMask[4] =
					{
							WhichX ? 0xFFFFFFFF : 0,
							WhichY ? 0xFFFFFFFF : 0,
							WhichZ ? 0xFFFFFFFF : 0,
			...
			template<uint32 Shuffle> struct PermuteHelper<Shuffle, false, false, false, false>
			{
			...
			template<uint32 Shuffle> struct PermuteHelper<Shuffle, true, true, true, true>
			{
			...
			template<uint32 Shuffle> struct PermuteHelper<Shuffle, false, false, true, true>
			{
			...
			template<uint32 Shuffle> struct PermuteHelper<Shuffle, true, true, false, false>
			{
		...
		****************************************************************************/
		...
		inline vec_float_t LUNA_SIMD_CALLCONV vector_set_binary_constant(uint32 C0, uint32 C1, uint32 C2, uint32 C3)
		{
#if defined(LUNA_PLATFORM_ARM_NEON)
		...
		inline vec_float_t LUNA_SIMD_CALLCONV vector_splat_constant(int32 intConstant, uint32 divExponent)
		{
		...
		inline vec_float_t LUNA_SIMD_CALLCONV vector_splat_constant_int(int32 intConstant)
		{
			luassert(intConstant >= -16 && intConstant <= 15);
#if defined(LUNA_PLATFORM_ARM_NEON)
