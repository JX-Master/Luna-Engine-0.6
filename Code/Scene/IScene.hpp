...
namespace Luna
{
	namespace Scene
	{
		...
		struct IScene : public Asset::IAsset
		{
			...
			virtual R<IEntity*> add_entity(const Name& entity_name) = 0;
			...
			virtual RV remove_entity(const Name& entity_name) = 0;

			//! Looks up one entity by its name.
			//! @param[in] name The name of the entity.
			virtual R<IEntity*> find_entity(const Name& name) = 0;
