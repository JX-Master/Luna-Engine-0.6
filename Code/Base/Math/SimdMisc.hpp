...
namespace luna
{
	namespace simd
	{
		...
		{
			vec_float_t T = VectorReplicate(t);
		...
		)
		{
			...
			vec_float SignMask = VectorSplatSignMask();
			...
			vec_float_t S1 = VectorSplatY(S0);
			S0 = VectorSplatX(S0);
		...
		{
			vec_float_t W = VectorSplatW(P);
			vec_float_t Z = VectorSplatZ(P);
			vec_float_t Y = VectorSplatY(P);
			vec_float_t X = VectorSplatX(P);
		...
		inline float32* LUNA_SIMD_CALLCONV PlaneTransformStream
		(
		...
		{
			vec_float_t r = VectorSplatX(rgb);
			vec_float_t g = VectorSplatY(rgb);
			vec_float_t b = VectorSplatZ(rgb);
		...
		inline vec_float_t LUNA_SIMD_CALLCONV ColorHSLToRGB(vec_float_p1_t hsl)
		{
			...
			vec_float_t s = VectorSplatY(hsl);
			vec_float_t l = VectorSplatZ(hsl);
			...
			{
				vec_float_t h = VectorSplatX(hsl);
		...
		{
			vec_float_t h = VectorSplatX(hsv);
			vec_float_t s = VectorSplatY(hsv);
			vec_float_t v = VectorSplatZ(hsv);
	...
	// G = sqrt(abs((RefractionIndex^2-1) + CosIncidentAngle^2))
		...
		)
		{
			...
			float32 fTemp = fabsf(Angle);
			fTemp = fTemp - (two_pi * static_cast<float32>(static_cast<int32>(fTemp / two_pi)));
		...
		inline float32 ScalarSin
		(
			float32 Value
		)
		{
			// Map Value to y in [-pi,pi], x = 2*pi*quotient + remainder.
			float32 quotient = one_div_two_pi * Value;
			if (Value >= 0.0f)
			{
				quotient = static_cast<float32>(static_cast<int>(quotient + 0.5f));
			}
			else
			{
				quotient = static_cast<float32>(static_cast<int>(quotient - 0.5f));
			}
			float32 y = Value - two_pi * quotient;
		...
		)
		{
			...
			float32 y2 = y * y;
			float32 p = ((-0.0012712436f * y2 + 0.041493919f) * y2 - 0.49992746f) * y2 + 1.0f;
			return sign * p;
		}
		...
		)
		{
			...
			float32 result = ((-0.0187293f*x + 0.0742610f)*x - 0.2121144f)*x + 1.5707288f;
			result *= root;  // acos(|x|)
