...
// IObject ID
#define luiid(x) static constexpr Luna::Guid __guid { Luna::Guid(u8##x) };
// Class ID
#define lucid(x) static constexpr Luna::Guid __guid { Luna::Guid(u8##x) };
...
namespace Luna
{
	...
	struct IObject
	{
		...
		//! This is the static `__guid` member of all interface types.
		...
		//! @return Returns `BasicError::bad_cast` if the object does not support the target interface
		...
		//! @return Returns `CoreError::expired` if the object is expired.
	...
	public:
		...
		{
			void* r;
			if (p->query_interface_raw(iid, &r).valid())
			{
				p = nullptr;
			}
			return r;
		...
		{
			_Rty* r;
			if (p->query_interface_raw(_Rty::__guid, (void**)&r).valid())
			{
				p = nullptr;
			}
			return r;
		...
		bool supports()
		{
			if (p->query_interface(_Rty::__guid, nullptr).valid())
			{
				return true;
			}
			return false;
		...
		P(const P<_Rty>& rhs)
		{
			...
			if (!rhs.empty())
			{
				auto _ = rhs->query_interface(_Ty::__guid, (void**)&p);
			}
		}
		template <typename _Rty>
		P(P<_Rty>&& rhs)
	...
	public:
		...
		P<_Ty> lock()
		{
			if (p)
			{
				if (!(p->add_ref_if_not_expired().valid()))
				{
					internal_clear();
				}
