...
namespace luna
{
	void vector_test()
	{
		...
		{
			// Default ctor.
			Vector<TestObject> vec1(get_global_heap());
...

		{
			...
			Vector<int> vec(get_global_heap());
			vec.resize(10, 7);
			vec[0] = 10;
			vec[1] = 11;
			vec[2] = 12;
			...
			Vector<TestObject> vec2(get_global_heap());
...

		{
			...
			Vector<int> vec2(get_global_heap());
...

		{
			...
			Vector<TestObject> vec(get_global_heap());
